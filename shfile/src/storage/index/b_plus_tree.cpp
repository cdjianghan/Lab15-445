...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) {
  ...
  buffer_pool_manager_->UnpinPage(neighbor_id, true);
  if (index == 0) {
    buffer_pool_manager_->DeletePage(neighbor_id);
  }
  return true;
  // 这几个释放逻辑要再想想
}
...
bool BPLUSTREE_TYPE::Coalesce(N **neighbor_node, N **node,
  ...
  if ((*node)->IsLeafPage()) {
    ...
    reinterpret_cast<InternalPage *>((*parent))->Remove(index2);
  } else {
    KeyType middle_key = reinterpret_cast<InternalPage *>((*parent))->KeyAt(index2);
