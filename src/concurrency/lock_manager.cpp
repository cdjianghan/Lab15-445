...
bool LockManager::LockShared(Transaction *txn, const RID &rid) {
    ...
    LockRequestQueue *lockRequestQueue =  &lock_table_.find(rid)->second;
    lockRequestQueue->request_queue_.emplace_back(txn->GetTransactionId(),LockMode::SHARED);
    if (lockRequestQueue->is_writing_) {
        lockRequestQueue->cv_.wait(lock,
                                   [lockRequestQueue,txn]()
                                           ->bool{
                                       return txn->GetState() == TransactionState::ABORTED ||
                                              !lockRequestQueue->is_writing_;
                                   });
    }
    if (txn->GetState() == TransactionState::ABORTED) {
        auto iter = GetIterator(lockRequestQueue->request_queue_,txn->GetTransactionId());
        lockRequestQueue->request_queue_.erase(iter);
        throw TransactionAbortException(txn->GetTransactionId(),AbortReason::DEADLOCK);
    }
    txn->GetSharedLockSet()->emplace(rid);
    GetIterator(lockRequestQueue->request_queue_,txn->GetTransactionId())->granted_ = true;
    lockRequestQueue->sharelock_count_++;
    return true;
...
bool LockManager::LockExclusive(Transaction *txn, const RID &rid) {
    ...
    if (lockRequestQueue->is_writing_ || lockRequestQueue->sharelock_count_ > 0){
        lockRequestQueue->cv_.wait(lock,
                                [lockRequestQueue, txn]()
                                        ->bool{ return txn->GetState() == TransactionState::ABORTED ||
                                                       (!lockRequestQueue->is_writing_ && lockRequestQueue->sharelock_count_ == 0);}
        );
    }
    if ( txn->GetState() == TransactionState::ABORTED) {
        auto iter = GetIterator(lockRequestQueue->request_queue_,txn->GetTransactionId());
...
bool LockManager::LockUpgrade(Transaction *txn, const RID &rid) {
    ...
    LockRequestQueue *lockRequestQueue =  &lock_table_.find(rid)->second;
    if (lockRequestQueue->upgrading_) {
        txn->SetState(TransactionState::ABORTED);
    ...
    if (lockRequestQueue->is_writing_ || lockRequestQueue->sharelock_count_ > 0){
        lockRequestQueue->upgrading_ = true;
        lockRequestQueue->cv_.wait(lock,
                                   [lockRequestQueue, txn]()
                                           ->bool{ return txn->GetState() == TransactionState::ABORTED ||
                                                          (!lockRequestQueue->is_writing_ && lockRequestQueue->sharelock_count_ == 0);}
    ...
    txn->GetExclusiveLockSet()->emplace(rid);
    lockRequestQueue->upgrading_ = false;
    iter->granted_ = true;
    lockRequestQueue->is_writing_ = true;
...
bool LockManager::Unlock(Transaction *txn, const RID &rid) {
    ...
    if (mode == LockMode::SHARED){
        if (--lockRequestQueue->sharelock_count_ == 0){
            lockRequestQueue->cv_.notify_all();
        }
    }
    else{
        lockRequestQueue->is_writing_ = false;
        lockRequestQueue->cv_.notify_all();
    }
...
//       In the handout they say 'throw a exception', but throw a exception
