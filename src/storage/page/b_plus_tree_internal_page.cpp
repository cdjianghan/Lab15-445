...
//  LOG_DEBUG("PopulateNewRoot:%ld",new_key.ToString());
...
//    LOG_DEBUG("InsertNodeAfter:%ld",new_key.ToString());
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyNFrom(MappingType *items, int size, BufferPoolManager *buffer_pool_manager) {
  ...
  for (int i = 0; i < size; i++) {
    array[i + lsize] = items[i];
    BPlusTreePage *page = reinterpret_cast<BPlusTreePage *>(buffer_pool_manager->FetchPage(items[i].second));
